...
ISR(TIMER1_OVF_vect)
{
  ...
  value = uart_getc();
if (value != '\0'){  
  uart_puts("u wanna get drunk?");
  uart_puts("\n");  
  if (value =='y'){
    uart_puts("lets go, thats what im talking about");
    if (value != '\0'){  
      uart_puts("we drinkin beer?");
      if (value == 'y'){
          uart_puts("i like beer");
      }
      else {
        uart_puts("so what are we drinking?");
      }
